...
namespace DataAccessLayer.Repositories
{
    public class BookRepository : IRepository<Book>
    {
        ...
        private static readonly string _path = Path.Combine(Directory.GetCurrentDirectory(), "books.txt");
        ...
        private string ConvertLine(Book book)
        {
            ...
            string year = book.PublishedYear.ToString().PadLeft(YEAR_LENGTH,'0');
            string categoryId = book.CategoryId.ToString().PadLeft(CATEGORY_ID_LENGTH,'0');
            //string memberId = (book.MemberId ?? 0)
            //.ToString()
            //.PadLeft(MEMBER_ID_LENGTH, '0');
            string memberId = (book.MemberId ?? 0).ToString().PadLeft(MEMBER_ID_LENGTH, '0');
            //string memberId = (book.MemberId ?? 0).ToString().PadLeft(MEMBER_ID_LENGTH, '0');
            string isAvailble =  book.IsAvailable ? "1" : "0";
            string separator = new string(' ', ID_SPACE);
            return id + separator + title + separator + author + separator + isbn + separator + year + separator + categoryId + separator + memberId + separator    + isAvailble;

        }

        private Book ParseFromLine(string line)
        {
            if (line.Length < ID_LENGTH + ID_SPACE + TITLE_LENGTH + AUTHOR_LENGTH + ISBN_LENGTH + YEAR_LENGTH + CATEGORY_ID_LENGTH + MEMBER_ID_LENGTH + IS_AVAILABLE_LENGTH)
            {
